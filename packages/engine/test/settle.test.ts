...
function makeSeat(spec: SeatSpec): Seat {
  return {
    ...
    // $1,000 in cents (money.ts). Settlement never reads it — it is here so a
    // `Seat` is well-formed — but a builder default that reads as $10 while
    // every stake around it is $10-$40 invites the wrong mental model.
    bankroll: 100_000,
...
const CASES: readonly SettlementCase[] = [
  ...
  { name: 'higher total wins', hand: { cards: 'TQ', bet: 1000 }, dealer: 'T9', outcome: 'win', payout: 2000, net: 1000 },
  { name: 'equal totals push', hand: { cards: 'T9', bet: 1000 }, dealer: 'Q9', outcome: 'push', payout: 1000, net: 0 },
  { name: 'lower total loses', hand: { cards: 'T8', bet: 1000 }, dealer: 'QT', outcome: 'lose', payout: 0, net: -1000 },
  {
  ...
  // Doubles. The state machine has already doubled `bet`, so nothing here scales it again.
  {
    ...
    hand: { cards: 'T63', bet: 2000, doubled: true },
    dealer: 'T8',
    outcome: 'win',
    payout: 4000,
    net: 2000,
  },
  {
    ...
    hand: { cards: 'T62', bet: 2000, doubled: true },
    dealer: 'T9',
    outcome: 'lose',
    payout: 0,
    net: -2000,
  },
  {
    ...
    hand: { cards: 'T6K', bet: 2000, doubled: true },
    dealer: 'T8',
    outcome: 'bust',
    payout: 0,
    net: -2000,
  ...
  },
  {
    ...
    hand: { cards: 'T63', bet: 2000, fromSplit: true, doubled: true },
    dealer: 'T7',
    outcome: 'win',
    payout: 4000,
    net: 2000,
  ...
  // Splits — 21 on a split hand is never a natural.
  {
    ...
    hand: { cards: 'AT', bet: 1000, fromSplit: true, fromSplitAces: true },
    dealer: 'T9',
    outcome: 'win',
    payout: 2000,
    net: 1000,
  },
  {
    ...
    hand: { cards: 'AQ', bet: 1000, fromSplit: true, fromSplitAces: true },
    dealer: 'T74',
    outcome: 'push',
    payout: 1000,
    net: 0,
  ...
  },
  {
    ...
    hand: { cards: 'TA', bet: 1000, fromSplit: true },
    dealer: 'TQ',
    outcome: 'win',
    payout: 2000,
    net: 1000,
  ...
  // Surrender.
  {
    ...
    hand: { cards: 'T6', bet: 1000, surrendered: true },
    dealer: 'T9',
    outcome: 'surrender',
    payout: 500,
    net: -500,
  ...
  },
  {
    ...
    hand: { cards: 'T6', bet: 500, surrendered: true },
    dealer: 'T9',
    outcome: 'surrender',
    payout: 250,
    net: -250,
...
describe('settleInsurance', () => {
  const seatWith = (insuranceBet: number): Seat =>
    makeSeat({ index: 2, hands: [{ cards: 'T6', bet: 1000 }], insuranceBet });
  ...
  it('pays 2:1 when the dealer has a natural', () => {
    expect(settleInsurance(seatWith(500), true, VEGAS_STRIP)).toEqual({
      seat: 2,
      bet: 500,
      payout: 1500,
      net: 1000,
  ...
  it('loses the stake when the dealer has no natural', () => {
    expect(settleInsurance(seatWith(500), false, VEGAS_STRIP)).toEqual({
      seat: 2,
      bet: 500,
      payout: 0,
      net: -500,
  ...
  it('exactly cancels a lost base bet — the break-even case that makes it a sucker bet', () => {
    ...
    const seat = seatWith(500);
    const insurance = settleInsurance(seat, true, VEGAS_STRIP);
    const hand = settleHand(makeHand({ cards: 'T6', bet: 1000 }), cardsOf('AK'), true, VEGAS_STRIP, 2, 0);
...
describe('settleRound', () => {
  it('settles seats in table order and hands in split order', () => {
    const state = makeState(
      [
        ...
        makeSeat({ index: 1, hands: [{ cards: 'TQ', bet: 1000 }] }),
        makeSeat({ index: 2, hands: [], occupant: { kind: 'bot', policyId: 'perfect', characterId: 'a' } }),
        makeSeat({
          index: 3,
          hands: [
            { cards: 'T8', bet: 1000, fromSplit: true },
            { cards: 'T9', bet: 1000, fromSplit: true },
            { cards: 'TK', bet: 1000, fromSplit: true },
          ],
  ...
  it('skips empty seats and seats that sat the round out', () => {
    const state = makeState(
      [
        ...
        makeSeat({ index: 2, hands: [{ cards: 'TQ', bet: 1000 }] }),
      ],
      'T9',
    );
  ...
  it('settles a four-hand split with mixed outcomes independently', () => {
    const state = makeState(
      [
        makeSeat({
          ...
          baseBet: 1000,
          hands: [
            { cards: 'A7', bet: 1000, fromSplit: true, fromSplitAces: true },
            { cards: 'AT', bet: 1000, fromSplit: true, fromSplitAces: true },
            { cards: 'T63', bet: 2000, fromSplit: true, doubled: true },
            { cards: 'T6K', bet: 1000, fromSplit: true },
          ],
        }),
      ],
      'TQ',
    ...
    expect(hands.map((h) => ({ outcome: h.outcome, payout: h.payout, net: h.net }))).toEqual([
      { outcome: 'lose', payout: 0, net: -1000 },
      // Even money: a split ace is never a natural, so no 3:2 here.
      { outcome: 'win', payout: 2000, net: 1000 },
      { outcome: 'lose', payout: 0, net: -2000 },
      { outcome: 'bust', payout: 0, net: -1000 },
    ]);
  });
  ...
  it('reports insurance only for the seats that took it', () => {
    const state = makeState(
      [
        makeSeat({ index: 0, hands: [{ cards: 'T6', bet: 1000 }], insuranceBet: 500 }),
        makeSeat({ index: 1, hands: [{ cards: 'T7', bet: 2000 }] }),
        makeSeat({ index: 2, hands: [{ cards: 'T8', bet: 4000 }], insuranceBet: 2000 }),
      ],
      'AK',
      true,
    );
    ...
    expect(insurance).toEqual([
      { seat: 0, bet: 500, payout: 1500, net: 1000 },
      { seat: 2, bet: 2000, payout: 6000, net: 4000 },
    ]);
